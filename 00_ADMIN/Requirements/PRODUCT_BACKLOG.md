...
1. Treat `10_SOURCE/Masters/Master 16/` with in-game label `Master 16.34` as the current governed production-test baseline.
...
7. The `Priority 1A - Medium Office Building Voxel Collapse` pattern is implemented through `Master 16.34`; validate that medium office buildings vary from 5 to 10 cubes per axis, use larger readable cubes, columns teeter then fall instead of holding a permanent lean, upper cubes wait for support failure, active cubes collide/separate instead of overlapping, faster cubes roll/bounce/impact nearby objects with more energy, cube audio stays readable, floor-entered cubes do not reappear on mobile, missed-hole cubes settle visibly, oversized cubes can jam/eject, and skyscrapers/houses keep their distinct behaviors.
...
- regression test `Master 16.34` through the modular package URL, then continue `PERF-012` Phase 2 as the next architecture slice.
