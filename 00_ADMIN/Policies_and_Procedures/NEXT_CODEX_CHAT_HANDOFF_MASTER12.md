...
- it adds validated mobile/audio polish:
  - mobile title music can start from the first mode-selection tap
  - music mutes when the tab/app loses focus
  - full-unit-clear speed boost is shortened from the earlier overpowered duration
  - mobile hole-wind audibility is increased
...
- `P1.8` is now in progress and `Master 12` carries the next validated instrumentation/audio-tuning-adjacent slice
...
- the user explicitly confirmed the mobile-wind-and-shorter-unit-boost candidate had no defects
- that build was promoted to `Master 12`
- current canonical baseline is therefore `Master 12`
...
2. Treat `Master 12` as the baseline
