...
- current in-game label: `Master 16.105`
...
- `QA-028` remains open: local governed files and review evidence now extend through `Master 16.105`, and this audit package refreshes the active handoff, but branch-visible publication is still pending for the dirty `NEW_CHAT_TEAM_SYNC_PROTOCOL.md` and `PRODUCT_BACKLOG.md` files because they already contain unrelated local edits
...
- treat `Master 16.105` as the current source basis within `10_SOURCE/Masters/Master 16/`
...
- validation focus:
  ...
  - validate `Master 16.104` mobile mode-selection interaction on a real device, plus `Master 16.105` build label, heavier building/destruction feel, grounded medium-voxel settle behavior, responsive Begin/world building, smooth sustained movement and dense consumption, intact audio/SFX after deferred decoding, readable Run Goals, active Mandate pressure/readability, boss visibility/behavior, randomized wave-start corners, corrected skyscraper debris direction, MegaKit Downtown/default Classic behavior, and no regression to validated building/destruction physics
