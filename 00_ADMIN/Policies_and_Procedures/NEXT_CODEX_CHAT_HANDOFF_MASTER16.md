...
- current in-game label: `Master 16.19`
...
- `Master 16.19` adds the separate How to Play field manual window while preserving the modular package shape
...
- immediate focus is regression testing `Master 16.19` through the modular package, then continuing `PERF-012` Phase 2 data/config extraction
...
- treat `Master 16.19` as the current source basis within `10_SOURCE/Masters/Master 16/`
- treat `index.html` as the package entry point only; production upload requires the full modular `/holesy/` folder contents
- start with regression testing `Master 16.19`, then continue `PERF-012` Phase 2
