...
| Current in-game build label | `Master 16.40` | Latest current patch label inside the approved Master 16 source package. |
...
| Current full GoDaddy upload folder | `C:\Users\KentLefner\Downloads\holesy-godaddy-upload-master-16.40\holesy\` | Full convenience copy for clean resync or rollback; not itself the source of truth. |
| Current GoDaddy delta upload folder | `C:\Users\KentLefner\Downloads\holesy-godaddy-delta-master-16.40-from-16.39\holesy\` | Default manual upload package when the live site already has the previous approved master. |
...
- A bundled `index.html` was a temporary publish artifact before `Master 16.17`; it is not the accepted architecture target. `Master 16.40` is the current modular patch on that architecture line.
