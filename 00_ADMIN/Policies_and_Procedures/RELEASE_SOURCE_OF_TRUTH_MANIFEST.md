...
| Current in-game build label | `Master 16.182` | Current internal review patch inside the approved Master 16 source and release package; normal player surfaces display `Version 16.182`. This candidate intentionally retains the temporary city override and is not release-clear. |
...
- A bundled `index.html` was a temporary publish artifact before `Master 16.17`; it is not the accepted architecture target. `Master 16.182` is the current modular review patch on that architecture line.
