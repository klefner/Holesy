...
| Current in-game build label | `Master 16.22` | Latest current patch label inside the approved Master 16 source package. |
...
| Current GoDaddy upload folder | `C:\Users\KentLefner\Downloads\holesy-godaddy-upload-master-16.22\holesy\` | Convenience copy for manual upload; not itself the source of truth. |
...
- A bundled `index.html` was a temporary publish artifact before `Master 16.17`; it is not the accepted architecture target. `Master 16.22` is the current modular help-window patch on that architecture line.
