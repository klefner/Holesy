...
| Current in-game build label | `Master 16.181` | Current internal review patch inside the approved Master 16 source and release package; normal player surfaces display `Version 16.181`. This candidate intentionally retains the temporary city override and is not release-clear. |
...
| Current Pages deployment branch | `claude/happy-clarke-ORWAI` | GitHub Pages publishes from this branch's `docs/` folder; commit `33dd0a3` publishes the directed-review build `Master 16.181`. |
...
- A bundled `index.html` was a temporary publish artifact before `Master 16.17`; it is not the accepted architecture target. `Master 16.181` is the current modular review patch on that architecture line.
