...
| Current in-game build label | `Master 16.212` | Current governed review build; normal player surfaces display `Version 16.212`. It preserves the complete Master 16.211 baseline while rebuilding Medieval cavalry as forward-aligned recognizable horses with articulated gait motion and correctly seated riders. |
...
| Current Pages deployment branch | `claude/happy-clarke-ORWAI` | GitHub Pages publishes from this branch's `docs/` folder; commit `933899b` publishes the complete `Master 16.211` package. A cache-busted public request on 2026-08-07 returned `Master 16.211`; local browser visual QA and console verification passed before publication. |
...
- A bundled `index.html` was a temporary publish artifact before `Master 16.17`; it is not the accepted architecture target. `Master 16.212` is the current modular review patch on that architecture line.
