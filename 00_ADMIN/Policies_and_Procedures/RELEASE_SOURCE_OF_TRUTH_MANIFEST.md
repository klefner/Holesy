...
| Current in-game build label | `Master 16.202` | Current release candidate; normal player surfaces display `Version 16.202`. It preserves the complete Master 16.201 content/performance baseline while adding Hole-Eye motion comfort, reliable first-person HUD teardown, and immediate-first-click Begin recovery. |
...
| Current Pages deployment branch | `claude/happy-clarke-ORWAI` | GitHub Pages publishes from this branch's `docs/` folder; commit `b02b8d8` publishes the complete `Master 16.201` package. Live verification on 2026-08-07 confirmed public build 201 metadata plus the live Harvest micro-detail governor, generic consume-pop path, and corresponding performance/audio telemetry. |
