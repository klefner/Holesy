...
| Current in-game build label | `Master 16.185` | Current internal review patch inside the approved Master 16 source and release package; normal player surfaces display `Version 16.185`. This candidate adds the Crownlands secret-town unlock contract and Archive clues, intentionally retains the temporary city override, and is not release-clear. |
...
- A bundled `index.html` was a temporary publish artifact before `Master 16.17`; it is not the accepted architecture target. `Master 16.185` is the current modular review patch on that architecture line.
