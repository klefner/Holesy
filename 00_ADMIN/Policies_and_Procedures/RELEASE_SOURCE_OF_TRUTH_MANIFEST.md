...
| Current in-game build label | `Master 16.198` | Current internal release candidate; normal player surfaces display `Version 16.198`. It preserves crowded Harvest food and intact authored buildings while applying compact destruction tiers, farmstead density clusters, a guaranteed starter trail, actor sleeping, field batching, supply-aware goals, and comparative performance telemetry. GitHub Pages publication is pending verification. |
...
| Current Pages deployment branch | `claude/happy-clarke-ORWAI` | GitHub Pages publishes from this branch's `docs/` folder; commit `7bbb88e` publishes the complete `Master 16.197` package. Live verification on 2026-07-31 confirmed HTTP 200, public 16.197 metadata, the restored 11x11 population grid and 96-worker rule, removal of the exact-object cap, explicit Mandate reward copy, and retained raised Hole-Eye camera constants. |
...
- A bundled `index.html` was a temporary publish artifact before `Master 16.17`; it is not the accepted architecture target. `Master 16.198` is the current modular review patch on that architecture line.
