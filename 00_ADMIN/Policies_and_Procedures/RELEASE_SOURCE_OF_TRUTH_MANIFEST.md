...
| Current in-game build label | `Master 16.59` | Latest current patch label inside the approved Master 16 source and release package after the 2026-06-09 source-of-truth drift repair audit. |
...
| Current full GoDaddy upload folder | `C:\Users\KentLefner\Downloads\holesy-godaddy-upload-master-16.52\holesy\` | Latest preserved full convenience copy currently visible in Downloads; it now lags the `Master 16.59` source basis and is not itself the source of truth. |
| Current GoDaddy delta upload folder | `C:\Users\KentLefner\Downloads\holesy-godaddy-delta-master-16.52-from-16.51\holesy\` | Current governed delta package root reported by Team Sync; it lags the `Master 16.59` source basis and should not be treated as current without refresh. |
...
- A bundled `index.html` was a temporary publish artifact before `Master 16.17`; it is not the accepted architecture target. `Master 16.59` is the current modular patch on that architecture line.
