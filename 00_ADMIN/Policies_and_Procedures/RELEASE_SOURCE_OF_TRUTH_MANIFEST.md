...
| Current in-game build label | `Master 16.202` | Current published review build; normal player surfaces display `Version 16.202`. It preserves the complete Master 16.201 content/performance baseline while adding Hole-Eye motion comfort, reliable first-person HUD teardown, and immediate-first-click Begin recovery. |
...
- A bundled `index.html` was a temporary publish artifact before `Master 16.17`; it is not the accepted architecture target. `Master 16.202` is the current modular review patch on that architecture line.
