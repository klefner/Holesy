...
| Current in-game build label | `Master 16.186` | Current internal review patch inside the approved Master 16 source and release package; normal player surfaces display `Version 16.186`. This candidate repairs gameplay audio, animal descent scale, and Medieval growth density, intentionally retains the temporary city override, and is not release-clear. |
...
| Current Pages deployment branch | `claude/happy-clarke-ORWAI` | GitHub Pages publishes from this branch's `docs/` folder; commit `a4489a3` publishes the directed-review build `Master 16.184`. Live verification on 2026-07-27 confirmed the 16.184 page and a Medieval Village boot with zero cars, hydrants, and electric lamps; 544 themed edibles; the ground-warband response profile; and the 6.2 Iron Reeve boss gate. |
...
- A bundled `index.html` was a temporary publish artifact before `Master 16.17`; it is not the accepted architecture target. `Master 16.186` is the current modular review patch on that architecture line.
