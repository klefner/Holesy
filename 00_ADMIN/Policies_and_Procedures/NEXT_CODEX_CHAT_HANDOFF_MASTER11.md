...
- `C:\Users\KentLefner\Desktop\game-repo\Holesy\10_SOURCE\Masters\Master 11.html`

Current basis note:

- `C:\Users\KentLefner\Desktop\game-repo\Holesy\10_SOURCE\Current\CURRENT_BASIS.md`

What `Master 11` specifically represents:

- it includes the full approved `Master 10` gameplay baseline
...
- it hardens reset / restart / wave-transition control so delayed next-wave callbacks do not resume gameplay after exit, restart, or game over
- it allows pause during the Waves transition interstitial and resumes the pending next-wave handoff cleanly
- it begins `P1.7` structure cleanup by centralizing repeated round UI state changes into explicit helpers
- it adds validated Waves payoff / pacing polish:
  - restored `60` second waves
  - final-3-seconds wave-end warning text
  - soldier unit-clear speed boost with immediate red/black rim flash
  - audible hole-wind ambience with long-tail pitch / volume drift
  - stronger Growth Cache mass reward
- it adds the first lightweight debug overlay foundation:
  - backtick toggle
  - live inspection of mode, wave, timer, radius, score, boosts, and wind state
  - proper cleanup on return to mode select

Last approved candidate promoted into `Master 11`:

- `C:\Users\KentLefner\Desktop\game-repo\Holesy\20_TESTS\Candidate_Builds\Master 10 - debug-overlay-foundation.html`
...
- `P1.8` is now in progress and its first validated slice is promoted into `Master 11`
...
- the user explicitly confirmed the debug-overlay-foundation candidate had no defects
- that build was promoted to `Master 11`
- current canonical baseline is therefore `Master 11`
...
2. Treat `Master 11` as the baseline
