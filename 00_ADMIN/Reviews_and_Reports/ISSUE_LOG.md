...
| QA-003 | Approved Master 15 lineage is local-only and not backed up on the working branch | GitHub completeness / Documentation governance | open | 2026-04-30 | 2026-05-10 | `CURRENT_BASIS.md`, `PRODUCT_BACKLOG.md`, and `NEXT_CODEX_CHAT_HANDOFF_MASTER15.md` now treat `10_SOURCE/Masters/Master 15.html` as the approved basis and `Master 15.8` as the current candidate, but `git log --oneline -10` still ends at branch head `39bbd94` and `git ls-files` shows tracked masters only through `Master 14`. `git status --short --branch` shows `Master 15.html`, `Master 14.10` through `Master 15.8` candidate files, and their QA reports are still local modified or untracked artifacts rather than tracked branch history. | Codex / Project Manager | yes | yes | Commit and push the approved `Master 15` promotion artifacts, the `Master 14.10` through `Master 15.8` candidate lineage, and the matching QA/governance reports, then verify those files appear in tracked branch history and `git status --short --branch` is clean for the preserved lineage. |
