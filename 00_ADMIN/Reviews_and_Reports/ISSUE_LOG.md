...
| QA-005 | Gameplay audio continues outside active play | Audio state / Gameplay QA | monitor | 2026-05-11 | 2026-05-11 | User reported failed audible validation for `Master 15.10` and `Master 15.11`; `Master 15.12` proved the countdown can now be heard, but the rising-pitch sound was rejected. `Master 15.13 - race-light-countdown-audio.html` keeps the direct audio/proof path and changes the countdown to three identical race-start-light style beeps, while retaining the `15.11` active-gameplay audio gates. | Codex / Project Manager | yes | yes | User must validate `15.13`: countdown is heard at 3, 2, and 1 seconds with the same tone each time; visible/debug cue proof appears; aid radar, soldier warning, gunshots, and plane engines stop when returning to game-mode selection or game-over. |
